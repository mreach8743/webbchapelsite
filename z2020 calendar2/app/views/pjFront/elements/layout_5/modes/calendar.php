...
<table id="phpevtcal_table_calendar" class="phpevtcal-table-calendar">
	...
	<tr class="phpevtcal-tr-nav">
  	...
  	<tr>
		<td colspan="9" class="phpevtcal-td-calendar">
			<table class="phpevtcal-inner-table">
...
for($row = 1; $row <= $num_rows; $row++)
{
	...
	<tr class="phpevtcal-tr-day">
		...
		for($col = 1; $col <= 7; $col++)
		{
			...
			{
				?><td class="date-cell last-month-day"><?php echo date('j', $run_day_timestamp);?></td><?php
			}else{
				if($run_day_timestamp <= $last_day_timestamp)
				{
					...
					if(!empty($event_date_arr[$run_date]))
					{
						...
						?>
						<td class="date-cell has-event" axis="<?php echo $day;?>" abbr="<?php echo $col;?>" lang="<?php echo $num_events;?>">
						...
						{
							?><td class="date-cell current-date"><?php echo date('j', $run_day_timestamp);?></td><?php
						}else{	
							?><td class="date-cell"><?php echo date('j', $run_day_timestamp);?></td><?php
						}
					}
				}else{
					?><td class="date-cell next-month-day"><?php echo date('j', $run_day_timestamp);?></td><?php
				}
			}

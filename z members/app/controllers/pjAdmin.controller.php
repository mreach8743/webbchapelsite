...
class pjAdmin extends pjAppController
{
	...
	public function pjActionIndex()
	{
		...
		if ($this->isAdmin() || $this->isEditor() || $this->isOwner())
		{
			...
			$cnt_members = $pjMemberModel->findCount()->getData();
			$cnt_active_members = $pjMemberModel->reset()->where('status', 'T')->findCount()->getData();
			$cnt_groups = $pjGroupModel->findCount()->getData();
			
			$last_registered_arr = $pjMemberModel->reset()->orderBy("created DESC")->limit(5, 0)->findAll()->getData();
			$last_login_arr = $pjMemberModel->reset()->where('last_login IS NOT NULL')->orderBy("last_login DESC")->limit(5, 0)->findAll()->getData();
			$popular_groups = $pjGroupModel->reset()->select('t1.*, (SELECT COUNT(*) FROM `'.$pjMemberModel->getTable().'` as t2 WHERE t2.group_id = t1.id) as cnt_members')
											->orderBy("cnt_members DESC")->limit(5, 0)->findAll()->getData();
			
			$this->set('cnt_members', $cnt_members);
			$this->set('cnt_active_members', $cnt_active_members);
			$this->set('cnt_groups', $cnt_groups);
			$this->set('last_registered_arr', $last_registered_arr);
			$this->set('last_login_arr', $last_login_arr);
			$this->set('popular_groups', $popular_groups);
